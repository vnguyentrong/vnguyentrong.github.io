...
post_asset: https://github.com/vnguyentrong/vnguyentrong.github.io/blob/icode_master/_posts/assets/2024-08-10-linux-memory-management-p1
---
...
The remain region of RAM is not mapped to kernel (because it is beyond the address range set aside for kernel virtual addresses). __Before accessing a specific high-memory page, the kernel must set up an explicit virtual mapping to make that page available in the kernel’s address. Thus, many kernel data structures must be placed in low memory; high memory tends to be reserved for user-space process pages__

![]({{page.post_asset}}/linux-memory.png)
